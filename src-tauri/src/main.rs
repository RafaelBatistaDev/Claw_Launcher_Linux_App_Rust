...
fn find_script() -> Result<PathBuf, String> {
    // Tenta usar CLAW_SCRIPT_DIR se definida
    ...
    // Tenta encontrar na mesma pasta do executável
    ...
    let common_paths = vec![
        PathBuf::from(std::env::var("HOME").unwrap_or_default()).join("OneDrive/App-Prontos/Claw_Launcher_Linux_App_Rust-main/create_app.sh"),
...
fn get_icon_base64(name: String) -> Result<String, String> {
    let script = find_script()?;
    let icon_path = script.parent()
        .ok_or("Erro ao localizar pasta pai")?
        .join("ICON")
        .join(format!("{}.png", name));

    if !icon_path.exists() {
        return Err("Ícone não encontrado".to_string());
    }
            ...
            if is_gui {
                ...
                window.on_window_event(move |event| {
                    if let tauri::WindowEvent::Focused(true) = event {
                        ...
                        window.__CLAW_POPUPS__ = new Map();
                        const originalWindowOpen = window.open;
                        window.open = function(url, target, features) {
                            if (!url) url = location.href;
                            if (!url.startsWith('http')) url = new URL(url, location.origin).href;
                        ...
                        document.addEventListener('click', function(e) {
                            ...
                            if (link.getAttribute('target') === '_blank') {
                                e.preventDefault();
                                if (link.href && link.href.startsWith('http')) location.href = link.href;
                            }
                        }, true);
                        ...
                        HTMLFormElement.prototype.submit = function() {
                            if (this.target === '_blank') {
                                ...
                                if (method.toUpperCase() === 'GET') {
                                    ...
                                    this.target = ''; originalFormSubmit.call(this); this.target = '_blank';
                                }
                                return;
                            }
                        ...
                        Object.defineProperty(navigator, 'userAgent', {
                            get: function() {
                                const ua = navigator.__UA__ || 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36';
                                return ua + ' CLAW-Browser/1.0';
                            }
                        });
                        "#;
