...
pub(crate) fn find_script() -> Result<PathBuf, String> {
    ...
    // 2. Tenta encontrar na mesma pasta do executável
    ...
    // 3. Tenta procurar em locais comuns (GoogleDrive ou OneDrive)
    let home = std::env::var("HOME").unwrap_or_default();
    let common_paths = vec![
        PathBuf::from(&home).join(".local/share/claw-launcher/create_app.sh"),
